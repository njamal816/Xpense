...
class XPenslyProBanner extends StatelessWidget {
  const XPenslyProBanner({this.large = false, this.fontColor, super.key});
  final bool large;
...
openManagePurchase() {
  if (appStateSettings["purchaseID"] == productIDs["lifetime"]) {
    ...
    openUrl(
        "https://play.google.com/store/account/subscriptions?sku=Xpense.pro.monthly&package=com.wpjp.xpensly");
  } else if (appStateSettings["purchaseID"] == productIDs["yearly"]) {
    openUrl(
        "https://play.google.com/store/account/subscriptions?sku=Xpense.pro.yearly&package=com.wpjp.xpensly");
  } else {
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 10),
            XPenslyProBanner(fontColor: getColor(context, "black")),
            TextFont(
...
showHelpRestorePopup(BuildContext context) {
  openPopup(
    ...
    onSubmit: () async {
      bool openResult = await openUrl('mailto:info@adamapps.in');
      if (openResult == false) copyToClipboard("info@adamapps.in");
    },
    onExtra: () =>
        openUrl("https://xpensly.adamapps.in/faq.html#restoring-purchases"),
    onExtraLabel: "FAQ".tr(),
  );
}
